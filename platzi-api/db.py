...
def consultar_carrera_por_id(id):
    return dumps(db.carreras.find_one({'_id': ObjectId(id)}))
...
def borrar_carrera_por_id(id):
    return str(db.carreras.delete_one({'_id': ObjectId(id)}))
...
def consultar_carreras(skip, limit):
    return dumps(db.cursos.find({}).skip(int(skip)).limit(int(limit)))
